...
task :uninstall do
  debug = str_to_boolean(ENV['debug'], :default => false)
  verbose = (debug || str_to_boolean(ENV['verbose'], :default => true))
  names = %w(.ackrc .bash .bash_profile .bashrc .bin .editrc .gemrc .gitignore .gvimrc.after .gvimrc.before .inputrc .irbrc .railsrc .vimrc.after .vimrc.before .zlogin .zsh .zshrc)
  replace_all = false
  names.each do |name|
